    ...
    return {
      ImportDeclaration(node) {
        ...
        for (let index = 1; index < node.specifiers.length; index++) {
          ...
          const areMultipleImportsOnSameLine =
            currentNode.loc.start.line === previousNode.loc.start.line;
          ...
          if (areMultipleImportsOnSameLine) {
            ...
            if (
              ...
              // E.g. import { useState, useEffect } from 'react'
              //                        ^
              const rangeAfterComma: TSESTree.Range = [comma.range[1], token.range[0]];
